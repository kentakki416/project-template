# step: Web に /api/dev/login と sign-in ボタンを追加

`apps/web` に dev-login の Route Handler、proxy 設定、sign-in 画面のボタンを追加する。

## 対応内容

### `apps/web/src/app/api/dev/login/route.ts`（新規）
...
export const GET = async (req: NextRequest) => {
  ...
  if (!as || !(as in DEV_USERS)) {
    return new NextResponse(`Usage: /api/dev/login?as=alice|bob`, { status: 400 })
  }
...
`/api/dev/login` を production 以外で PUBLIC_PATHS に追加する。
  ...
  <div className="space-y-2 border-t border-dashed border-gray-200 pt-4">
    ...
    <div className="flex gap-2">
      {DEV_LOGIN_USERS.map((user) => (
        <Link href={`/api/dev/login?as=${user}`} key={user} className="...">
          Login as {user}
        </Link>
      ))}
    </div>
...
4. `http://localhost:3000/api/dev/login?as=bob` を直接踏んでも同じく `/` にリダイレクトされる
5. 不正な値 `http://localhost:3000/api/dev/login?as=charlie` は 400 + 案内文を返す
6. `verify-web-page` skill で sign-in 画面の before/after スクショを取って PR に貼る

`NODE_ENV=production pnpm build && pnpm start` でビルドし、`/api/dev/login?as=alice` が 404、sign-in 画面に dev ボタンが出ないことを確認する。
