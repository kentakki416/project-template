 ...
 * package.json の test スクリプトで DB_NAME=project-template_test を設定済みなので、
...
```json
{
  "scripts": {
    ...
    "test": "DB_NAME=project-template_test dotenvx run -f .env.local -- pnpm --filter @repo/db db:migrate:deploy && vitest run",
    "test:ci": "pnpm --filter @repo/db db:generate && pnpm --filter @repo/db db:migrate:deploy && vitest run",
    "test:coverage": "DB_NAME=project-template_test dotenvx run -f .env.local -- pnpm --filter @repo/db db:migrate:deploy && vitest run --coverage"
  }
}
```
