...
- **グローバルステート用Context**: プロップドリリングの回避
...
- **早すぎる最適化（Premature Optimization）**: 早期に最適化しすぎる
- **NIH症候群（Not Invented Here）**: 既存のソリューションを拒否
- **分析麻痺（Analysis Paralysis）**: 計画しすぎ、構築が不十分
- **マジック（Magic）**: 不明確で文書化されていない動作
- **密結合（Tight Coupling）**: コンポーネント間の依存が強すぎる
...
**覚えておくこと**: 良いアーキテクチャは、迅速な開発、容易な保守、自信を持ったスケーリングを可能にします。最良のアーキテクチャはシンプルで明確であり、確立されたパターンに従います。
