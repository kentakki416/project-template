...
terraform {
  backend "s3" {
    bucket         = "project-template-terraform-state" # TODO: bootstrap/variables.tfと同じ値に変更
    key            = "dev/terraform.tfstate"
    region         = "ap-northeast-1"
    dynamodb_table = "project-template-terraform-state-lock" # TODO: bootstrap/variables.tfと同じ値に変更
    encrypt        = true
  }
}
